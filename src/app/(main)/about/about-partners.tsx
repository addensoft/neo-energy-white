...
/**
 * AboutPartners — a condensed restatement of the Trust & Technology Bar's
 * partner marks, for visitors who land on About without ever having scrolled
 * the homepage. Same official logo assets and mask treatment (see
 * `sections/trust-bar/partner-mark.tsx`); reveal is a plain `RevealWrapper`
 * stagger here rather than that section's GSAP entrance timeline, since this
 * page has no equivalent chapter-reveal machinery to hook into.
 *
 * `"use client"`: `PartnerMark` is itself a Client Component requiring a
 * function `innerRef` prop, and a Server Component can't pass a plain
 * closure across that boundary (functions aren't serialisable RSC payload) —
